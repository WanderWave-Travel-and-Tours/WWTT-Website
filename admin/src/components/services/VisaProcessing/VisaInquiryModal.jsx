...
const VisaInquiryModal = ({ isOpen, onClose, inquiry, refreshData }) => {
  ...
  const handleUpdateStatus = async (status) => {
    try {
        const response = await axios.put(
            `https://wanderwaveph-backend.onrender.com0/api/inquiries/${inquiry._id}/status`,
            { status }
        );
        if (response.data.success) {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
        const response = await axios.put(`https://wanderwaveph-backend.onrender.com0/api/inquiries/${inquiry._id}/confirm-payment`, {
            adminName: 'Admin' 
        });
        if (response.data.success) {
  ...
  const handleDeliverDocuments = async () => {
    ...
    try {
        ...
        const response = await axios.put(
            `https://wanderwaveph-backend.onrender.com0/api/inquiries/${inquiry._id}/deliver-documents`,
            formData,
            { headers: { 'Content-Type': 'multipart/form-data' } }
        );
                  ...
                  <div className="vim-file-list" style={{ marginBottom: '20px' }}>
                    ...
                    {finalSentDocs.map((doc, idx) => (
                      <div key={idx} className="vim-file-item" style={{ 
                        ...
                        <a href={`https://wanderwaveph-backend.onrender.com0${doc.fileUrl}`} target="_blank" rel="noopener noreferrer" 
                ...
                <div className="vim-doc-list">
                  {clientDocs.map((doc) => (
                    <div key={doc._id} className="vim-doc-item">
                      ...
                      <a href={`https://wanderwaveph-backend.onrender.com0${doc.fileUrl}`} download={doc.originalName} className="vim-btn vim-btn-ghost vim-btn-sm">
                        <Upload size={14} style={{transform: 'rotate(180deg)'}} /> Download
                      </a>
                      <a href={`https://wanderwaveph-backend.onrender.com0${doc.fileUrl}`} target="_blank" rel="noopener noreferrer" className="vim-btn vim-btn-ghost vim-btn-sm">
                        <TrendingUp size={14} /> View
                      </a>
                    </div>
                  ))}
