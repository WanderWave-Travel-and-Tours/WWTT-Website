...
const RequirementEditor = ({ selectedVisa, onBack, onSave }) => {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            const res = await axios.post('/api/visas/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' }});
            if (res.data.success) {
...
const VisaSettingsModal = ({ isOpen, onClose, visaForms, isLoading, countryCodes, refreshData }) => {
    ...
    const handleSaveRequirements = async (updatedData) => {
        try {
            await axios.put(`/api/visas/${selectedVisa.id}`, updatedData);
            refreshData();
            setViewMode("LIST");
        } catch (error) {
    ...
    const handleAddVisa = async () => {
        ...
        try {
            await axios.post("/api/visas/add", {
