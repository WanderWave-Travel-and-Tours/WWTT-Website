...
const RequirementEditor = ({ selectedVisa, onBack, onSave }) => {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            const res = await axios.post('https://wanderwaveph-backend.onrender.com0/api/visas/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' }});
            if (res.data.success) {
...
const VisaSettingsModal = ({ isOpen, onClose, visaForms, isLoading, countryCodes, refreshData }) => {
    ...
    const handleSaveRequirements = async (updatedData) => {
        try {
            await axios.put(`https://wanderwaveph-backend.onrender.com0/api/visas/${selectedVisa.id}`, updatedData);
            refreshData();
            setViewMode("LIST");
        } catch (error) {
    ...
    const handleDeleteVisa = async (id) => {
        ...
        try {
            await axios.delete(`https://wanderwaveph-backend.onrender.com0/api/visas/${id}`);
            refreshData();
    ...
    const handleAddVisa = async () => {
        ...
        try {
            await axios.post("https://wanderwaveph-backend.onrender.com0/api/visas/add", {
