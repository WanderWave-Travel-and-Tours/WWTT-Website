...
const PSASerbilis = () => {
  ...
  const fetchPSADocs = async () => {
    try {
      const res = await axios.get("https://wanderwaveph-backend.onrender.com0/api/psa");
  ...
  const handleUpdateInquiryStatus = async (inquiryId, newStatus) => {
    try {
      const response = await axios.put(
        `https://wanderwaveph-backend.onrender.com0/api/inquiries/${inquiryId}/status`,
        { status: newStatus }
      );
  ...
  const submitContactWithRemarks = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://wanderwaveph-backend.onrender.com0/api/inquiries/${selectedInquiry._id}/status`,
        formData,
  ...
  const handleSaveService = async () => {
    ...
    try {
        if (selectedPSA) {
            // Update
            await axios.put(`https://wanderwaveph-backend.onrender.com0/api/psa/${selectedPSA.id}`, payload);
            alert("Changes saved successfully!");
        } else {
            // Create
            await axios.post("https://wanderwaveph-backend.onrender.com0/api/psa", payload);
  ...
  const handleDeletePSA = async (id) => {
    if (window.confirm("Delete this service?")) {
        await axios.delete(`https://wanderwaveph-backend.onrender.com0/api/psa/${id}`);
        fetchPSADocs();
    }
  };
  ...
  const handleDirectFileUpload = async (e) => {
    ...
    try {
      const response = await axios.post('https://wanderwaveph-backend.onrender.com0/api/psa/upload', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
