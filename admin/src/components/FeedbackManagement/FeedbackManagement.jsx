...
const FeedbackManagement = () => {
  ...
  const fetchFeedbacks = async () => {
    setLoading(true);
    try {
      const response = await fetch('https://wanderwaveph.onrender.com/api/feedback', {
  ...
  const handleArchive = (id) => {
    ...
    setConfirmConfig({
      ...
      onConfirm: async () => {
        try {
          const response = await fetch(`https://wanderwaveph.onrender.com/api/feedback/${id}/archive`, {
            method: 'PATCH',
            headers: getAuthHeaders()
          });
