...
export const exportWeeklyToPDF = (stats, weeklyData = [], topPackages = []) => {
    try {
        const doc = new jsPDF();
        ...
        drawCard('WEEKLY REVENUE', 'P' + (stats?.todayRevenue || 0).toLocaleString(), [139, 92, 246]);
        ...
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.text('2. WEEKLY REVENUE BREAKDOWN', 20, yPos + 5.5);
        yPos += 12;
        ...
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.text('4. WEEKLY PERFORMANCE ANALYTICS', 20, yPos + 5.5);
        yPos += 12;

        doc.setFontSize(10);
        doc.text('WEEKLY REVENUE TRAJECTORY', 14, yPos + 5);
        ...
        // IN-ADJUST: Nilipat sa pageWidth / 2 + 40 para hindi mag-overlap
