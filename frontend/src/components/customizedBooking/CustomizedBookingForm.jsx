...
export default function CustomizedBookingForm({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (step !== 2 || (availableTours.length && availableTransfers.length)) return;
    fetchServices();
  }, [step]);
              ...
              <div className="cbf-form-grid">
                ...
                <div className="cbf-field">
                  ...
                  <div className="cbf-input-wrap">
                    ...
                    <input
                      type="date"
                      className={infoErrors.travelDate ? 'cbf-error' : ''}
                      value={info.travelDate}
                      min={new Date().toISOString().split('T')[0]}
                      onChange={e => setInfo(p => ({...p, travelDate: e.target.value}))}
                    />
                  </div>
                ...
                <div className="cbf-field">
                  ...
                  <div className="cbf-input-wrap">
                    ...
                    <input
                      ...
                      min={info.travelDate || new Date().toISOString().split('T')[0]}
                      onChange={e => setInfo(p => ({...p, returnDate: e.target.value}))}
                    />
                  </div>
                </div>
...
// ─────────────────────────────────────────────────────────────────────────────
function TransferList({ transfers, selected, transferTypes, onToggle, onTypeChange }) {
