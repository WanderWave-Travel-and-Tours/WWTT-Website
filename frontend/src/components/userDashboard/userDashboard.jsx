...
const UserDashboard = ({ user, onLogout }) => {
    ...
    const fetchUserData = async () => {
        ...
        try {
            const inquiriesPromise = fetch(`https://wanderwaveph-backend.onrender.com0/api/inquiries/email/${user.email}`).then(res => res.json());
            
            const bookingsPromise = fetch(`https://wanderwaveph-backend.onrender.com0/api/bookings/user/${user.email}`).then(res => res.json());
    ...
    useEffect(() => {
        ...
        if (isSuccess === 'true' && inquiryId) {
            const verifyPayment = async () => {
                try {
                    setIsLoading(true);
                    await axios.put(`https://wanderwaveph-backend.onrender.com0/api/inquiries/${inquiryId}/pay`);
    ...
    useEffect(() => {
        const fetchVisaDetails = async () => {
            if (selectedInquiry?.visaId) {
                try {
                    const response = await axios.get(`https://wanderwaveph-backend.onrender.com0/api/visas/${selectedInquiry.visaId}`);
    ...
    const handlePayment = async () => {
        ...
        try {
            setIsLoading(true);
            const response = await fetch('https://wanderwaveph-backend.onrender.com0/api/payment/create-inquiry-checkout', {
                method: 'POST',
    ...
    const submitDocuments = async () => {
        ...
        try {
            await fetch('https://wanderwaveph-backend.onrender.com0/api/documents/upload', { method: 'POST', body: formData });
